...
export function LoginView() {
  ...
  const handleOAuthLogin = async (provider: string) => {
    try {
      ...
      // Note: target_app_url must match an entry in ALLOWED_FRONTEND_ORIGINS configured on the Auth N&Z backend
      // (e.g. https://tasks.l4s3r.site), otherwise login will fall back to default origin.
      const targetAppUrl = window.location.origin;
      const res = await api.getOAuthLoginUrl(provider, redirectUri, targetAppUrl);
